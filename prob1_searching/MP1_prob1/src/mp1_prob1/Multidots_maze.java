...
public class Multidots_maze {
	...
	public int multidots_normal_SolveMazeAStar(Maze cmaze) {
		...
		int n_goals = -1;
		// System.out.println("mazesize" + cmaze.get_width() *
		// cmaze.get_height());
		...
		while (n_goals != n_goal_found) {
			...
			while (curr != start_state) {
				pathcost++;
				if (cmaze.maze_index(curr % cmaze.get_width(),
				...
				} else if (cmaze.maze_index(curr % cmaze.get_width(), curr
						/ cmaze.get_width()) == ' ')
					cmaze.fill_in_maze(curr % cmaze.get_width(),
							curr / cmaze.get_width(), '*');
				// System.out.println("cur" + curr);
	...
	public int multidots_SolveMazeAStar(Maze cmaze) {
		...
		while (n_goals != n_goal_found) {
			while (mc_que.size() != 0) {
				...
				if ((testret != 0 && rotate_flag == 0)
						|| (testret != 0 && rotate_flag == 1 && start_state != temp
								.get_index())) {

					retval = testret;
					center = temp.get_index();
					// System.out.println("center" + center + " retval" +
					// retval);
					rotate_count = 0;
					goal_state = center;
			...
			while (curr != start_state) {
				...
				if (cmaze.maze_index(curr % cmaze.get_width(),
					...
					if (pathcost < 9)
						cmaze.fill_in_maze(curr % cmaze.get_width(), curr
								/ cmaze.get_width(), 'A');
					else if (pathcost < 21) {
						cmaze.fill_in_maze(curr % cmaze.get_width(), curr
								/ cmaze.get_width(), 'B');
			...
			if (rotate_flag != 0 && rotate_count != 4) {
				goal_state = cmaze
						.find_rotate_pos(retval, rotate_count, center);
				rotate_count++;
				if (rotate_flag == 2) {
					System.out.println();
			...
			// find the nearest goal again
				...
				for (int i = 0; (goal_found = cmaze.find_goals(cur_pos)) != -1; i++) {
					int cur_Mdist = cmaze.manhattan_distance(
							goal_found % cmaze.get_width(),
							goal_found / cmaze.get_width(),
							start_state % cmaze.get_width(), start_state
									/ cmaze.get_width());
					multidot_goal_cell goal_cell = new multidot_goal_cell(
							goal_found, cur_Mdist);
				...
				chosengoal = gc_que.poll();
				if (cmaze.may_have_wall_between(start_state,
						chosengoal.get_pos())) {
					int mid = (start_state + chosengoal.get_pos()) / 2;
					if (cmaze.maze_index(mid % cmaze.get_width(),
							mid / cmaze.get_width()) == '%') {
						if (gc_que.size() != 0)
							chosengoal = gc_que.poll();
					}
			...
			for (int i = 0; i < cmaze.get_width() * cmaze.get_height(); i++) {
				...
				// if(goal_state == 43)
				// {
				// System.out.println("index" + i + " heu "
				// +cmaze_cell[i].get_heuristic());
				// }
			}
			mc_que.clear();
			mc_que.add(cmaze_cell[start_state]);
